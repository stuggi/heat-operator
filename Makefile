...
test: manifests generate gowork fmt vet envtest ginkgo ## Run tests.
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) --bin-dir $(LOCALBIN) -p path)" $(GINKGO) --trace --cover --coverpkg=./internal/...,./api/v1beta1/... --coverprofile cover.out --covermode=atomic ${PROC_CMD} $(GINKGO_ARGS) ./test/... ./internal/...
##@ Build
...
force-bump: ## Force bump operator and lib-common dependencies
	for dep in $$(cat go.mod | grep openstack-k8s-operators | grep -vE -- 'indirect|heat-operator|^replace' | awk '{print $$1}'); do \
		go get $$dep@$(BRANCH) ; \
	done
	for dep in $$(cat api/go.mod | grep openstack-k8s-operators | grep -vE -- 'indirect|heat-operator|^replace' | awk '{print $$1}'); do \
